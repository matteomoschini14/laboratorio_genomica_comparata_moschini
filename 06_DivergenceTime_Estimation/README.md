...
On TimeTree.org, the [Dataset.tsv](../00_start/Dataset.tsv) dataset can be uploaded; however, it is crucial to ensure that the first column is modified so that underscores are replaced by spaces. If underscores are retained, the database will fail to recognize the taxonomic names.
